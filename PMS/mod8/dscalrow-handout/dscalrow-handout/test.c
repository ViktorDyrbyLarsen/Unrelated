...
int main(void)
{
    ...
    if (dscalrow(2.0, &A, 3) != 1) // k is out of bounds
    {
        printf("  ***Test failed. Unexpected return code when index is out of bounds.\n");
        return EXIT_FAILURE;
    }
    if (dscalrow(2.0, NULL, 1) != 1) // A is NULL
    {
    ...
    double Ar_expected[] = {1,2,3,4,10,12,14,16,9,10,11,12};
    ...
    double Ac_expected[] = {1,2,6,4,5,12,7,8,18,10,11,24};
    for (size_t i = 0; i < A.shape[0] * A.shape[1]; i++)
    {
