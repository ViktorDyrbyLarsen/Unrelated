...
int main(void)
{
    ...
    */
    ...
    // Compute y = A*x
    printf("Testing spmv() with valid inputs...\n");
    spmv(&A, x, y);

    // Check result
    double yref[3] = {20,15,50};
    ...
    spmv(&A, x, NULL); // This will result in a segfault if spmv() does not handle NULL input correctly.
    spmv(&A, NULL, y); // This will result in a segfault if spmv() does not handle NULL input correctly.
    spmv(NULL, x, y);  // This will result in a segfault if spmv() does not handle NULL input correctly.
