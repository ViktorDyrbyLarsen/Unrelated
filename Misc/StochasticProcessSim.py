...
def ornstein_uhlenbeck_process(gamma, X_0, T, N, mu=0.0, sigma=1.0):
    # Minimal fix: actually update X each step using Euler–Maruyama
    dt = T / N
    ...
    for i in range(1, N):
        dW = sqrt_dt * torch.randn(1, device=device)
        X[i] = X[i-1] + gamma * (mu - X[i-1]) * dt + sigma * dW
...
Z = geometric_brownian_motion(100, 0.0, 0.2, T, N)
plot_process(Z, title='Geometric Brownian Motion', ylabel='Z(t)')

plot_process(torch.log(geometric_brownian_motion(100, 0.0, 0.2, T, N)),
             title='Log Geometric Brownian Motion', ylabel='log Z(t)')

# --- OU example (added) ---
X_ou = ornstein_uhlenbeck_process(gamma=3.0, X_0=2.0, T=T, N=N, mu=0.0, sigma=0.5)
plot_process(X_ou, title='Ornstein–Uhlenbeck Process (Euler–Maruyama)', ylabel='X(t)')
